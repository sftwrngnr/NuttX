...
void input_acknack(Session* session, OutputReliableStream* output_stream, const uint16_t first_unacked_seq_num, uint8_t bitmap[2])
{
    /* Clear buffers */
    int i;
    for(i = seq_num_add(output_stream->last_acknown, 1); 0 > seq_num_cmp(i, first_unacked_seq_num); i = seq_num_add(i, 1))
    {
    ...
    /* Send lost */
    for(i = 0; i < MICRORTPS_MAX_MSG_NUM; i++)
    {
...
void output_acknack(InputReliableStream* input_stream, ACKNACK_Payload* acknack)
{
  uint16_t i;
    ...
    for(i = 0; i < search_buffers_size; i++)
    {
